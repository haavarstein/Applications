...
If (!(Test-Path -Path $Source)) {
    Invoke-WebRequest -UseBasicParsing -Uri $url -OutFile RDA.zip
    Expand-Archive -Path RDA.zip -DestinationPath .
    Remove-Item -Path RDA.zip
         }
        Else {
...
Copy-Item $Source -Destination C:\Windows\System32
